<div id="mainliste">
	...
	<?php
			...
			$sql_list = "SELECT ka.id, ka.charname, kl.name as klassenname, kl.color, kl.symbol, a.name as angebotsname, a.preis, a.info  
			FROM kaeufer ka LEFT JOIN klassen kl ON ka.klassen_id = kl.id
				LEFT JOIN angebote a ON ka.angebot_id = a.id";
			...
			echo "<td id=\"d" . $c["id"] . "\" class=\"tdlist\">" . utf8_encode($c["angebotsname"]) . "</td>";
			echo "<td id=\"e" . $c["id"] . "\" class=\"tdlist\">" . $c["preis"] . "</td>";
			echo "<td id=\"f" . $c["id"] . "\" class=\"tdlist\">" . utf8_encode($c["info"]) . "</td>";
